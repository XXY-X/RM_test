...
{
	//      һȦ   ڣ   ֹ         δ      µĿ
	referee_data.decoder.receive_data_arr = REFEREE_RECVBUF_SIZE - DMA_GetCurrDataCounter(REFEREE_RECV_DMAx_Streamx); //  ȡ   е ǰ Ѿ    յĳ

	//      Ȧ
	if (referee_data.decoder.judgementFullCount - referee_data.decoder.decodeFullCount > 1)
	{
		referee_data.decoder.decodeFullCount = referee_data.decoder.judgementFullCount - 1;
	}
	u8 byte;

	while (referee_data.decoder.decodeFullCount < referee_data.decoder.judgementFullCount || referee_data.decoder.decode_data_arr < referee_data.decoder.receive_data_arr - 1) //  û    ʱ
	{
		byte = Refereebuffer[referee_data.decoder.decode_data_arr]; //     ֽ ȡ
		...
		referee_data.decoder.decode_data_arr++;
		if (referee_data.decoder.decode_data_arr >= REFEREE_RECVBUF_SIZE)
		{
			referee_data.decoder.decode_data_arr = 0;
			referee_data.decoder.decodeFullCount++;
		}
		//   ½   λ
		referee_data.decoder.receive_data_arr = REFEREE_RECVBUF_SIZE - DMA_GetCurrDataCounter(REFEREE_RECV_DMAx_Streamx); //  ȡ   е ǰ Ѿ    յĳ
	}
