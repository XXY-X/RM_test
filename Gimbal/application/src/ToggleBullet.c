...
void selectShootFreq(uint8_t robot_level, uint8_t shoot_buff)
{
    ...
    // 反转
    if (toggle_controller.rev_shoot)
    {
        toggle_controller.shoot_freq_speed = 500 * (-1.0f);
    }
}
...
void TogglePidInit()
{
    ...
    PID_Init(&toggle_controller.toggle_speed_pid, 5.0, 3.0f, 0, 0.026, 0.02, 0, 0, 0, 0, 0, 1, Integral_Limit | Trapezoid_Intergral);
}
