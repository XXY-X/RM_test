...
void GimbalPidInit()
{
    ...
    PID_Init(&gimbal_controller.pitch_speed_pid, GM6020_MAX_VOLTAGE, 11000, .0f, 300.0f, 120.0f, 0, 0, 0, 0.0, 0, 1, Integral_Limit | Trapezoid_Intergral);

    // yaw GM6020 MOTOR VOL LOOP
    PID_Init(&gimbal_controller.yaw_angle_pid, 280.0, 0, 0, 20.0f, 0, 0, 0, 0, 0.0, 0, 1, NONE);
    PID_Init(&gimbal_controller.yaw_speed_pid, GM6020_MAX_VOLTAGE, 9000, 0.0, 360.0f, 15.0f, 0, 0, 0, 0.0018, 0, 1, Integral_Limit | Trapezoid_Intergral);
...
{
    float cur_motor_angle = gimbal_controller.pitch_info.angle;
