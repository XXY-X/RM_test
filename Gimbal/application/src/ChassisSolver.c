...
void DJIKeyMouseUpdate(ChassisSolver *infantry)
{
    ...
    else
    {
        ...
        // 鼠标操作
        if (remote_controller.gimbal_action == GIMBAL_ACT_MODE || remote_controller.gimbal_action == GIMBAL_TEST_MODE || remote_controller.gimbal_action == GIMBAL_AUTO_AIM_MODE)
        {
        ...
        if (press_l)
        {
            ...
            toggle_controller.rev_shoot = ctrl_flag == TRUE ? TRUE : FALSE;
        }
        else
        {
        ...
        if (press_r && remote_controller.gimbal_action == GIMBAL_ACT_MODE)
        {
            ...
            pc_send_data.aim_request = TRUE;
        }
        else
        {
            pc_send_data.aim_request = FALSE;
        }
...
void DJIRemoteUpdate(ChassisSolver *infantry)//形参实际上没什么鸟用
{
 ...
 * @param[in] infantry
 */
void get_control_info(ChassisSolver *infantry)//根据控制模式选择不同的控制命令解析函数
{
    switch (remote_controller.control_type)//会解析出控制模式，云台yaaw和pitch的目标角度，以及底盘的各方向速度
    {
    case BLUE_TOOTH:
