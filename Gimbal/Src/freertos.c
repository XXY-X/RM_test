...
void MX_FREERTOS_Init(void)
{
  ...
  // Action Task
  osThreadDef(ActionTask_, ActionTask, osPriorityRealtime, 0, 1024);
  ActionTaskHandle = osThreadCreate(osThread(ActionTask_), NULL);

  // Gimbal Task
  osThreadDef(GimbalTask_, GimbalTask, osPriorityRealtime, 0, 1024);
  GimbalTaskHandle = osThreadCreate(osThread(GimbalTask_), NULL);

  // Offline Task
  osThreadDef(Offline_task_, Offline_task, osPriorityRealtime, 0, 1024);
  OfflineTaskHandle = osThreadCreate(osThread(Offline_task_), NULL);

  // Chassis Task
  osThreadDef(ChassisTask_, ChassisTask, osPriorityRealtime, 0, 1024);
